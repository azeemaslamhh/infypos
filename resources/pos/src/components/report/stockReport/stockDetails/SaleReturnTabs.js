...
const SaleReturnTabs = (props) => {
    const {
        ...
        stockDetailSaleReturn, id, allConfigData
    } = props;
    ...
    const itemsValue = currencySymbol && stockDetailSaleReturn.length >= 0 && stockDetailSaleReturn.map(sale => ({
        ...
        id: sale.id,
        sale_return_items: sale.attributes.sale_return_items.map((item) => ({
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            cell: row => {
                return (row.sale_return_items.map((item) => item.product_id === Number(id) ? item.name : ''))
            }
        },
        {
        ...
        },
        {
            ...
            sortable: false,
            cell: row => {
                return (row.sale_return_items.map((item) => item.product_id === Number(id) ?
        ...
        },
        {
            ...
            sortable: false,
            cell: row => {
                return (row.sale_return_items.map((item) => item.product_id === Number(id) ? currencySymbolHendling(allConfigData, row.currency, item.sub_total) : ''))
            }
        },
    ];
...
const mapStateToProps = (state) => {
    const {isLoading, totalRecord, stockDetailSaleReturn, frontSetting} = state;
