...
const SaleTabs = (props) => {
    const {
        ...
        stockDetailsSales, id, allConfigData
    } = props;
    ...
    const itemsValue = currencySymbol && stockDetailsSales.length >= 0 && stockDetailsSales.map(sale => ({
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            cell: row => {
                return (row.sale_items.map((item) => item.product_id === Number(id) ? item.name : ''))
            }
        },
        {
        ...
        },
        {
            ...
            sortable: false,
            cell: row => {
                return (row.sale_items.map((item) => item.product_id === Number(id) ?
        ...
        },
        {
            ...
            sortable: false,
            cell: row => {
                return (row.sale_items.map((item) => item.product_id === Number(id) ? currencySymbolHendling(allConfigData, row.currency, item.sub_total) : ''))
            }
        },
    ];
